...
int main(int argc,char** argv){
    ...
    qsort(data, handle_num, sizeof(float), cmpfunc);
    ...
    while(!allSort){
        ...
        if(isEven && rank!=last_rank){
            ...
            if(data[handle_num-1] > data_in[0]){ // sort the array
                ...
                // mergeArray2(data,data_in,data_tmp,&handle_num,&recv_len);
...
void mergeArray2(float *arr1, float *arr2, float *arr_out, int *l1, int *l2){
    int tmp_len = *l1 + *l2;
    int ptr = 0;
    for(ptr=0;ptr<*l1;ptr++){
        arr_out[ptr] = arr1[ptr];
    }
    for(int j=0;j<*l2;j++){
        arr_out[ptr] = arr2[j];
        ptr++;
    }
    qsort(arr_out,tmp_len,sizeof(float),cmpfunc);
}
